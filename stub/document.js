...
class HTMLInputElement extends HTMLElement {
  ...
  get value() {
    return typeof this.generatorFn === 'function'
      ? this.generatorFn()
      : this.generatorFn;
  }
  set value(value) {
...
function querySelector(selector) {
  ...
  const cachedElem = (elemArgs, ElementType) => {
    ...
    querySelector.DOM[cacheSelector] =
      typeof querySelector.DOM[cacheSelector] === 'undefined'
        ? new ElementType(elemArgs)
        : querySelector.DOM[cacheSelector];
  ...
  switch (selector) {
    ...
    case '.diameter':
      return cachedElem(
        { selector, value: () => Math.ceil(Math.random() * 100) },
        HTMLInputElement
      );
    case '.output':
